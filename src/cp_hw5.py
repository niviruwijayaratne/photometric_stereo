...
def integrate_frankot(zx, zy):
    ...
    Wx = np.tile(2*np.pi/H2*np.hstack((np.arange(0, H2/2+1),
                 np.arange(-H2/2+1, 0))).reshape(-1, 1), (W2, 1)).flatten()
    Wy = np.kron(2*np.pi/W2*np.hstack((np.arange(0, W2/2+1),
                 np.arange(-W2/2+1, 0))).reshape(-1, 1), np.ones((H2, 1))).flatten()
